var Arch = function (params) {
  ...
  this.position = params.position || { x: 0, y: 0, z: 0};
    ...
    this.radius = params.radius || Ball.RADIUS + 2;
      ...
      var box_increment_angle = Math.PI / (2 * this.no_of_boxes); //math.
      var x_len = this.radius * Math.tan(box_increment_angle);// anti-overlap
    
      if (!this.box_autowidth) {
        x_len = this.box_width; //set a width in case the box doesn't have one
      } 
    
      var shape = new CANNON.Box(new CANNON.Vec3(x_len, this.box_height, this.box_thickness));
    
      for (var i = 0; i < this.no_of_boxes; i++) {  //creating each box
